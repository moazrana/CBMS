...
const Index=()=>{
    ...
    const [negativeSafeguards,setNegativeSafeguards]=useState<safeguard[]>([]);
    const [poitiveSafeguards,setPoitiveSafeguards]=useState<safeguard[]>([]);
    const [safeguardingSafeguards,setSafeguardingSafeguards]=useState<safeguard[]>([]);
    ...
    const fetchSafeguards=async()=>{
        ...
        res.forEach((element:any) => {
            ...
            safe.student=element.student.name
            safe.location=element.location.name
            safe.eventTime=element.dateAndTime
            safe.period=element.period.name
            safe.description=element.description
            safe.staff=element.staff.name
            safe.status=element.status
            safe.data=element
            setSafeguards(perv=>[...perv,safe])
        });
    ...
    const openSafeguard=async(safeguard:safeguard)=>{
        ...
        setDescription(safeguard.data.description)
        setSeverity(safeguard.data.commentary.severity)
        setIdt(safeguard.data.commentary.direction)
        setBehavior(safeguard.data.commentary.behaviour)
        setMeetings(safeguard.data.meetings)
        setConcernTypes(safeguard.data.type)
        setAccountChecks(safeguard.data.your_account)
        setBodyMap(safeguard.data.body_mapping)
        setSocialCare(safeguard.data.early_help)
        setRefferal(safeguard.data.referral_type)
        setConclusion(safeguard.data.conclusion)

        setLinkForSafeguard('/safeguarding/safeguard/'+safeguard.data._id)
        setOpenRS(true)
    }
                    ...
                    {view=='component'&&(<>
                        <div className="compo-view">
                            <div className="comps-div">
                                ...
                                {safeguards.map((safeguard,index)=>(
                                    <>
                                        ...
                                        <ListComponent
                                            key={index}
                                            student={safeguard.student}
                                            subject={safeguard.subject}
                                            eventTime={safeguard.eventTime}
                                            description={safeguard.description}
                                            location={safeguard.location}
                                            status={safeguard.status}
                                            onClick={()=>{
                                                openSafeguard(safeguard)
                                            }}
                                        />    
                                    </>
                                ))}
                            ...
                            <div className="comps-div">
                                <ComponentHeader
                                    secName='Negative'
                                    count={negativeSafeguards.length}
                                />
                                {negativeSafeguards.map((safeguard,index)=>(
                                    <>
                                    {safeguard.status}
                                        <ListComponent
                                            key={index}
                            ...
                            <div className="comps-div">
                                <ComponentHeader
                                    secName='Positive'
                                    count={poitiveSafeguards.length}
                                />
                                {poitiveSafeguards.map((safeguard,index)=>(
                                    <>
                                    {safeguard.status}
                                        <ListComponent
                                            key={index}
                            ...
                            <div className="comps-div">
                                <ComponentHeader
                                    ...
                                    count={safeguardingSafeguards.length}
                                />
                                {safeguardingSafeguards.map((safeguard,index)=>(
                                    <>
                                    {safeguard.status}
                                        <ListComponent
                                            key={index}
