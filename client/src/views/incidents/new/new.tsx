...
const New=()=>{
    ...
    const handleSave = async () => {
        try {
            ...
            const url = '/incidents';
            const method = 'post';
            await executeRequest(method, url, formData, { headers: { 'Content-Type': 'multipart/form-data' } }).then(res=>{
                if(res.status===200){
                    navigate('/incidents')
                }
            });
    ...
    const openIncident=async(incident:any)=>{
        setStudent(incident.student._id)
        setStaff(incident.staff._id)
        setDescription(incident.description)
        setSeverity(incident.commentary.severity)
        setIdt(incident.commentary.direction)
        setBehavior(incident.commentary.behaviour)
        setMeetings(incident.meetings)
        setConcernTypes(incident.type)
        setAccountChecks(incident.your_account)
        setBodyMap(incident.body_mapping)
        setSocialCare(incident.early_help)
        setRefferal(incident.referral_type)
        setConclusion(incident.conclusion)
    }
    ...
    useEffect(() => {
        ...
        getUsers('Student')
        getUsers('Staff')
        if(url.pathname.includes('incident')){
            getIncident()
            return
        }
    ...
    return (
        ...
        >
            <div className="main-div">
                ...
                <div className="all-inputs-div-incident">
